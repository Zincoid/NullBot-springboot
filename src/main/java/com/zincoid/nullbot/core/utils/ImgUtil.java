...
public final class ImgUtil {
    ...
    public static String compressDataUri(String dataUri) {
        ...
        if (head < 0 || !dataUri.startsWith("data:image/")) return dataUri;
        if (dataUri.startsWith("data:image/gif;")) return dataUri;
        String body = dataUri.substring(head + 1);
        byte[] image = Base64.getDecoder().decode(body);
        if (body.length() <= COMPRESS_THRESHOLD) return dataUri;
        try {
            BufferedImage source = ImageIO.read(new ByteArrayInputStream(image));
            if (source == null) return dataUri;
            ...
            try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
                if (!ImageIO.write(target, "jpg", out)) return dataUri;
