...
public class CommandEvent<T extends Event> {
    ...
    public static CommandEvent<?> of(String command, boolean authRequired) {
        List<String> information = List.of(command.split(" "));
        return CommandEvent.of(BotCtx.getEvent(), information.getFirst(), information.subList(1, information.size()), authRequired, false);
    }

    public static CommandEvent<?> of(String command) {
        return of(command, BotCtx.getScope() == EventScope.GROUP && BotCtx.getSetting().isInnerCmdAuth());
    }
}
