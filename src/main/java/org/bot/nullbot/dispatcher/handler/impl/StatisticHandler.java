...
public class StatisticHandler implements Handler
{
    ...
    public void handle(Bot bot, Command command, CommandEvent<?> event, CommandHandlerChain chain) throws Exception {
        if(event.getEvent() instanceof GroupMessageEvent groupMessageEvent){
            ...
            statisticService.increase(groupMessageEvent.getGroupId(), groupMessageEvent.getSender().getUserId(), groupMessageEvent.getSender().getNickname(), command.getClass().getSimpleName());
            log.info("\t\t├─[StatisticHandler] 记录完成");
