...
- 增强 聊天历史存储：
  ...
  - 支持 File 存储；
  - 完善相关代码；
- 增强 History 指令 - 支持图片查看；
- 增强 Recalled 指令 - 支持图片防撤回；
- 优化 指令名；
- 优化 配置结构。
...
- 优化 AI 欠费提醒方式；
- 优化 Question 指令 - 添加 AI 欠费拦截；
- 优化 ImageSave 指令 - 存复数图片时提示总数；
- 增强 SayingSave 指令 - 支持合并转发消息的保存；
- 重构 梗图生成 - 合并为 Generate 指令；
- 修复 AA 指令截图异常。

### 2026.9.2

- 重构 WebCapturer - 优化为链式调用接口；
- 新增 AA 指令 - 添加相关截图服务功能；
- 移除 Essence 指令；
- 优化 Chrome 组件 - 改简化方法并添加无元素异常处理；
- 优化 QQMsgExecutor - 拦截内部指令异常避免反复发送异常。
...
- 增强 SayingSave 命令 - 新增AI的语录保存支持。

### 2026.6.23

- 优化 AuthHandler 和 HelpHandler 的通用指令参数格式；
- 优化 帮助菜单文档和部分代码。

### 2026.6.22

- 添加 KeywordReacter 组件 - 替换 BotMonitor 的关键词检测逻辑；
- 优化 指令嵌入参数 - 使用 {} 标记模板占位；
- 优化 部分文本。
...
- 新增 Usage 指令 - 查询个人指令用量；
- 新增 Rps 指令 - 猜拳游戏；
- 优化 包结构；
- 简化 工具类名；
- 简化 指令文档；
- 增强 CommandRegistry 组件 - 支持大小写敏感配置项；
...
- 优化 MsgParseUtil - @机器人解析为@你；
- 增强 SysMsgSet 指令 - 添加查看功能；
- 增强 QQMsgExecutor - 添加多种过滤模式；
- 优化 AI - 拒绝和过滤回复使用不同图片；
...
- 优化系统向 QQ 的错误响应形式：
    - 新增 CommandArgs 类 - 用于封装指令参数；
    - 修改 ExecutorHandler::execute 方法签名；
    - 变更 异常组织结构；
    - 修改 所有指令的错误响应；
    - 优化 所有指令；
- 优化 临时文件夹不再分层；
...
- 优化 EmbedCommandEvent：
    ...
    - 优化构造方法和相关使用；
- 优化 枚举类属性为全大写；
- 新增 群设置属性 ChatStrategy 枚举类：
    - 用于控制群聊对话策略并修改相关代码；
    - 私聊默认策略为 EMBEDDING；
- 修改 群设置属性 custom 逻辑：
    - 仅自定义模式下限权0的用户可修改提示词；
    - 移除提示词管理器冗余逻辑，优化相关代码；
- BaseMessage 新增 reasoningContent 字段：
    - 用于应对 Dsv4 思考模式工具调用需求；
    - 修改相关代码；
- 工具调用策略已支持思考和语音模式；
- 记忆存储添加 ReentrantLock 锁机制防止并发问题；
- 修复 历史截断导致工具消息造成异常的问题；
- 新增 内部指令 PokeCommand；
- 优化 部分代码和日志，移除日志缩进。
...
- 增强 BotCtxUtil：
    ...
    - 新增 ChatId 获取方法；
- 优化 QQAiClient - 优化方法参数列表和代码结构；
- 优化 Model::invoke 方法返回类型为 Message。
...
- 优化 CommandListener 和 MonitorListener - 优化结构，方法签名和日志输出；
...
- 优化 所有 getHelpForAI 方法 - 移除限权信息和部分指令帮助；
- 优化 项目结构。
...
- [ 重要！] 修复 AsyncConfig 配置 - 修复 ThreadExecutor Bean 未自动关闭线程池使得 Restarter 无法完全结束当前应用进程进而导致多次通过
  Jar 重启应用后产生大量 Java 进程造成卡死的问题。
...
- 网页Bot管理系统 后端增强 LoginController 的 getInfo方法 - 额外返回用户类型 用于 前端禁用按钮 (仅为美观
  实际已在后端拦截)；
...
- [ 重要！] 修复 同一消息事件并行调用 CommandListener 与 MonitorListener 相关多个监听方法时存在严重冲突的问题 (尤其在 AI
  Monitor 模式下)。
